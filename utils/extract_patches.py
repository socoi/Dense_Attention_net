import numpy as np
import random
from utils.preprocessing import load_hdf5, my_PreProc
...
def extract_random(full_imgs, full_masks, patch_h, patch_w, N_patches, inside=True):
    ...
    patches_masks = np.empty(
        (N_patches, full_masks.shape[1], patch_h, patch_w))
    ...
    for i in range(full_imgs.shape[0]):  # loop over the full images
        ...
        while k < patch_per_img:
            x_center = random.randint(
                0 + int(patch_w / 2), img_w - int(patch_w / 2))
            # print "x_center " +str(x_center)
            y_center = random.randint(
                0 + int(patch_h / 2), img_h - int(patch_h / 2))
    ...
    for i in range(full_imgs.shape[0]):  # loop over the full images
        for h in range(N_patches_h):
            for w in range(N_patches_w):
                patch = full_imgs[i, :, h * patch_h:(
                    h * patch_h) + patch_h, w * patch_w:(w * patch_w) + patch_w]
                patches[iter_tot] = patch
                iter_tot += 1  # total
...
def extract_ordered_overlap(full_imgs, patch_h, patch_w, stride_h, stride_w):
    ...
    N_patches_img = ((img_h - patch_h) // stride_h + 1) * ((img_w -
                                                            patch_w) // stride_w + 1)  # // --> division between integers
...
def get_data_training(DRIVE_train_imgs_original,
    ...
    patches_imgs_train, patches_masks_train = extract_random(
        train_imgs, train_masks, patch_height, patch_width, N_subimgs, inside_FOV)
    data_consistency_check(patches_imgs_train, patches_masks_train)

    # , patches_imgs_test, patches_masks_test
...
def get_data_testing_overlap(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width, stride_height, stride_width):
    ...
    test_imgs = my_PreProc(test_imgs_original) #proprecessing
    test_masks = test_masks / 255.
    #extend both images and masks so they can be divided exactly by the patches dimensions
    test_imgs = test_imgs[0:Imgs_to_test, :, :, :]
    test_masks = test_masks[0:Imgs_to_test, :, :, :]
...
def recompone(data, N_h, N_w):
    ...
    full_recomp = np.empty(
        (int(N_full_imgs), data.shape[1], N_h * patch_h, N_w * patch_w))
    k = 0  # iter full img
...
def paint_border(data, patch_h, patch_w):
    ...
    new_data = np.empty((data.shape[0], data.shape[1], new_img_h, new_img_w))
